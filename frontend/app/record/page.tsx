...
export default function RecordPage() {
  ...
  return (
    <div className="min-h-screen bg-background px-6 py-12 max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold text-foreground mb-2">Live Recording</h1>
      <p className="text-muted-foreground mb-10">Record audio and generate AI notes instantly.</p>

      <div className="flex flex-col items-center gap-6 mb-10">
        <div className={`relative flex items-center justify-center w-28 h-28 rounded-full cursor-pointer transition-all
          ${recording ? "bg-red-500/20 border-2 border-red-500" : "bg-card border-2 border-border hover:border-accent"}`}
          onClick={recording ? stopRecording : startRecording}>
          {recording
            ? <Square className="w-10 h-10 text-red-400" />
            : <Mic className="w-10 h-10 text-card-foreground" />}
          {recording && <div className="absolute inset-0 rounded-full border-2 border-red-500 animate-ping opacity-30" />}
        </div>
        <div className="font-mono text-3xl text-card-foreground">{fmt(timer)}</div>
        <p className="text-muted-foreground text-sm">{recording ? "Recording… tap to stop" : "Tap to start recording"}</p>
      </div>

      {transcript && (
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-card-foreground font-semibold">Transcript</h2>
          ...
          <div className="p-4 rounded-xl bg-card border border-border text-card-foreground text-sm leading-relaxed">
            {transcript}
          </div>
        </div>
      )}
